localizator.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

localizator.utils.getMenu = function (actions, grid, selected) {
    const menu = [];
...
localizator.utils.renderActions = function (value, props, row) {
    const res = [];
    ...
    return String.format(
        '<ul class="localizator-row-actions">{0}</ul>',
        res.join('')
    );
};