    ...
    function installPackage($packageName)
    {
        ...
        if ($responseBody !== '') {
            ...
            if ($foundPackages === false) {
                return array(
                    'success' => 0,
                    'message' => "Invalid response from repository for <b>{$packageName}</b>.",
                );
            }
...
// MIGX не в списке: нужен только при наличии TV типа "migx"; пользователь ставит отдельно при необходимости
$packages = array();
$success = false;
...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            $modx->log(\modX::LOG_LEVEL_INFO, '== InstallDependencies: migrating');
            foreach ($packages as $name => $version) {
                ...
                foreach ($installed as $package) {
                    if ($package->compareVersion($version, '<=')) {
                        $modx->log(\modX::LOG_LEVEL_INFO, "✓ {$name} already installed (>= {$version})");
                        continue(2);
                    }
                }
                ...
                if (is_array($response) && !empty($response['success'])) {
                    $modx->log(\modX::LOG_LEVEL_INFO, "✓ Installed {$name}");
                } else {
                    $msg = is_array($response) && isset($response['message']) ? $response['message'] : "Could not install {$name}";
                    $modx->log(\modX::LOG_LEVEL_WARN, "✗ " . $msg . " (optional dependency)");
                }
            }
            $elapsed = number_format(microtime(true) - $tStart, 4);
            $modx->log(\modX::LOG_LEVEL_INFO, '== InstallDependencies: migrated ' . $elapsed . 's');
            $success = true;
            break;
